...
let make = () => {
  ...
  <div className="rescript-test-component">
    ...
    <button onClick={_ => setCount(prev => prev + 1)} className="btn waves-effect blue">
      {React.string("Increment")}
    </button>
    <p className="green-text"> {React.string("✓ ReScript compilation working!")} </p>
    <p className="blue-text"> {React.string("✓ Hot Module Replacement working!")} </p>
  </div>
}

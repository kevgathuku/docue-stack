...
function decodeDocument(json) {
  ...
  let match$2 = Stdlib_JSON.Decode.string(ownerId);
  if (match === undefined) {
    return {
      TAG: "Error",
  ...
  let contentOpt = content !== undefined ? Stdlib_JSON.Decode.string(content) : undefined;
  let roleOpt;
  if (role !== undefined) {
    let match$3 = Stdlib_JSON.Decode.$$null(role);
    roleOpt = match$3 !== undefined ? undefined : Stdlib_JSON.Decode.string(role);
  } else {
    roleOpt = undefined;
  }
  let dateCreatedOpt = dateCreated !== undefined ? Stdlib_JSON.Decode.string(dateCreated) : undefined;
  let lastModifiedOpt = lastModified !== undefined ? Stdlib_JSON.Decode.string(lastModified) : undefined;
  return {
    TAG: "Ok",
    _0: {
      _id: match,

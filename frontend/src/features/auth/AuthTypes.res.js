...
function decodeUserName(json) {
  ...
  let match = Core__JSON.Decode.string(first);
  let match$1 = Core__JSON.Decode.string(last);
...
function decodeUser(json) {
  ...
  let match = Core__JSON.Decode.string(id);
  let match$1 = Core__JSON.Decode.string(username);
  let match$2 = decodeUserName(name);
  let match$3 = Core__JSON.Decode.string(email);
  if (match === undefined) {
    return {
      TAG: "Error",
  ...
  let roleOpt;
  if (role !== undefined) {
    let match$4 = Core__JSON.Decode.$$null(role);
  ...
  let loggedInOpt = loggedIn !== undefined ? Core__JSON.Decode.bool(loggedIn) : undefined;
  return {
    TAG: "Ok",
    _0: {
      _id: match,

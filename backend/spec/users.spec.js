describe('User Spec', () => {
  ...
  describe('User Documents', () => {
    it("should get a user's documents", async () => {
      const doc = await Documents.find({}).limit(1).exec();
  ...
  describe('User Session', () => {
    it('should return false if no token is provided', async () => {
      const res = await request(app).get('/api/users/session');
    ...
    it('should return true if the user is logged in', async () => {
      const res = await request(app).get('/api/users/session').set('x-access-token', token);
    ...
    it('should return false if the user is logged out', async () => {
      // logout the user
      const logoutRes = await request(app).post('/api/users/logout').set('x-access-token', token);

      expect(logoutRes.statusCode).toBe(200);

      const sessionRes = await request(app).get('/api/users/session').set('x-access-token', token);
